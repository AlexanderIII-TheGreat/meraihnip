    ...
    <>
      ...
      <div className="md:hidden fixed inset-0 z-50 bg-[#f4f7fa] overflow-y-auto overflow-x-hidden pb-24">
        ...
        <div className="p-4">
          ...
          {isPembahasan && (
            <div className="mb-6">
              {/* Status bar: benar/salah */}
              {soal?.jawabanShow && (() => {
                const jawabanBenar = soal.jawabanShow.find((j: any) => j.isCorrect);
                const jawabanDipilih = soal.jawabanShow.find((j: any) => j.id === selected);
                const isBenar = jawabanBenar?.id === selected;
                return (
                  <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl mb-4 ${
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                      !selected ? 'bg-gray-300' : isBenar ? 'bg-green-500' : 'bg-red-500'
                    }`}>
                      {!selected ? (
                    ...
                    <div>
                      <p className={`text-xs font-bold uppercase tracking-wide ${
                        !selected ? 'text-gray-500' : isBenar ? 'text-green-700' : 'text-red-700'
                      }`}>
                        {!selected ? 'Tidak Dijawab' : isBenar ? 'Jawaban Benar!' : 'Jawaban Salah'}
                      </p>
                      {!isBenar && jawabanBenar && (
                        <p className="text-[11px] text-gray-500 mt-0.5 font-medium">
                          Jawaban benar: <span className="font-bold text-green-600">
                            {String.fromCharCode(65 + soal.jawabanShow.findIndex((j: any) => j.isCorrect))}
                          </span>
                        </p>
                      )}
                    </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white">
                  ...
                  <div>
                    <h3 className="font-bold text-[#1e293b] text-sm">Pembahasan</h3>
                    <p className="text-[10px] text-gray-400 font-medium">Penjelasan jawaban soal ini</p>
                  </div>
                </div>
                ...
                <div className="p-5">
                  {soal?.pembahasan ? (
                    <div className="prose prose-sm max-w-none text-[#1e293b] leading-relaxed tryout-mobile-ck">
                      <CKeditor content={soal.pembahasan} readOnly />
                    </div>
                  ) : (
                    <div className="flex flex-col items-center py-6 text-center">
                      ...
                      <p className="text-sm text-gray-400 font-medium">Pembahasan belum tersedia</p>
                      <p className="text-[11px] text-gray-300 mt-1">untuk soal ini</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                return (
                  <button
                    ...
                    className={`w-12 h-12 shrink-0 rounded-xl flex items-center justify-center font-bold text-sm transition-all ${
                      isActive
                        ? 'bg-[#1e1b4b] text-white shadow-lg ring-2 ring-[#1e1b4b] ring-offset-2'
                        : isPembahasan
                        ? item.status === 'BENAR'
            ...
            {isPembahasan ? (
              <div className="flex items-center gap-4 mt-2 flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-gray-500 font-bold">Benar</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-full bg-red-500"></div>
                  <span className="text-[10px] text-gray-500 font-bold">Salah</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-full bg-gray-200"></div>
                  <span className="text-[10px] text-gray-500 font-bold">Tidak Dijawab</span>
                </div>
              </div>
            ) : (
