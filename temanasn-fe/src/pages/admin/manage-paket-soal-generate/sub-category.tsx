...
import ManageCategory from './manage-sub';
...
export default function ManageSoalGenerateSub() {
  ...
  const fetchParentDetail = async () => {
    try {
      const res: any = await getData(`admin/parent-generate-soal-category/find/${parentId}`);
      if (res && res.name) {
        setParentName(res.name);
      }
    } catch (error) {
      console.error('Failed to fetch parent detail', error);
    }
  };
  ...
  return (
    <section className="flex justify-center">
      ...
      <div className="p-8 pt-2 rounded-2xl max-w-5xl w-full">
        <BreadCrumb
          page={[
            { name: 'Manage Soal Generate', link: '/manage-soal-generate' },
            { name: `Sub-Kategori: ${parentName}`, link: '#' },
          ]}
        />
          ...
          <div className="flex flex-col gap-y-5 md:flex-row md:items-center justify-start md:justify-between header-section w-full">
            <div className="title border-b border-[#ddd] w-full flex justify-between items-center">
              <h1 className="text-2xl text-indigo-950 font-bold mb-5 ">
                Manage Sub-Kategori: {parentName}
              </h1>
              <div className="flex items-center gap-6 mb-5">
                <Button
                  ...
                  onClick={() => {
                    setDetail({ parentId: Number(parentId) });
                    setVisible(true);
                  }}
                >
                  <IconPlus
