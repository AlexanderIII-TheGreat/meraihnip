...
const insert = async (req, res, next) => {
  try {
    ...
    // Audit Log for Create Paket Pembelian
    if (req.user) {
      const logData = {
        id: req.user.id,
        name: req.user.name,
        email: req.user.email,
        url: `Buat Paket Pembelian: ${result.nama}`,
        ip: req.ip,
        last_activity: Date.now(),
        action: 'CREATE_PAKET_PEMBELIAN',
      };
...
const update = async (req, res, next) => {
  try {
    ...
    // Audit Log for Edit Paket Pembelian
    if (req.user) {
      const logData = {
        id: req.user.id,
        name: req.user.name,
        email: req.user.email,
        url: `Ubah Paket Pembelian: ${result.nama}`,
        ip: req.ip,
        last_activity: Date.now(),
        action: 'EDIT_PAKET_PEMBELIAN',
      };
      await redis.rpush('audit_logs', JSON.stringify(logData));
      await redis.ltrim('audit_logs', -1000, -1);
    }
...
const remove = async (req, res, next) => {
  try {
    ...
    // Audit Log for Delete Paket Pembelian
    if (req.user) {
      const logData = {
        id: req.user.id,
        name: req.user.name,
        email: req.user.email,
        url: `Hapus Paket Pembelian: ${isExist.nama || validate.id}`,
        ip: req.ip,
        last_activity: Date.now(),
        action: 'DELETE_PAKET_PEMBELIAN',
      };
      await redis.rpush('audit_logs', JSON.stringify(logData));
      await redis.ltrim('audit_logs', -1000, -1);
    }
