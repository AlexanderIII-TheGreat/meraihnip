...
const user = async (req, res, next) => {
  try {
    ...
    // Fetch latest news
    const berita = await database.berita.findMany({
      take: 5,
      orderBy: {
        createdAt: 'desc',
      },
    ...
    // Compute Tryout Saya (type: BIASA) stats
    const totalTryoutSaya = await database.tryout.count({
      where: { userId: req.user.id, paketLatihan: { type: 'BIASA' } }
    });
    const selesaiTryoutSaya = await database.tryout.count({
      where: { userId: req.user.id, finishAt: { not: null }, paketLatihan: { type: 'BIASA' } }
    });
    const aggregateSaya = await database.tryout.aggregate({
      where: { userId: req.user.id, finishAt: { not: null }, paketLatihan: { type: 'BIASA' } },
      _sum: { point: true, maxPoint: true }
    });
    const avgSaya = aggregateSaya._sum.maxPoint ? Math.round((aggregateSaya._sum.point / aggregateSaya._sum.maxPoint) * 100) : 0;

    // Compute Tryout Akbar (type: TRYOUT) stats
    const totalTryoutAkbar = await database.tryout.count({
      where: { userId: req.user.id, paketLatihan: { type: 'TRYOUT' } }
    });
    const selesaiTryoutAkbar = await database.tryout.count({
      where: { userId: req.user.id, finishAt: { not: null }, paketLatihan: { type: 'TRYOUT' } }
    });
    const aggregateAkbar = await database.tryout.aggregate({
      where: { userId: req.user.id, finishAt: { not: null }, paketLatihan: { type: 'TRYOUT' } },
      _sum: { point: true, maxPoint: true }
    });
    const avgAkbar = aggregateAkbar._sum.maxPoint ? Math.round((aggregateAkbar._sum.point / aggregateAkbar._sum.maxPoint) * 100) : 0;

    const tryoutSayaStats = {
      total: totalTryoutSaya || 0,
      selesai: selesaiTryoutSaya || 0,
      rataRata: avgSaya || 0,
      target: 10
    };

    const tryoutAkbarStats = {
      total: totalTryoutAkbar || 0,
      selesai: selesaiTryoutAkbar || 0,
      rataRata: avgAkbar || 0,
      target: 5
    };
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const dateStr = dayNames[d.getDay()];
    ...
    const totalSeconds = await database.tryout.aggregate({
      where: {
        userId: req.user.id,
        createdAt: {
          gte: sevenDaysAgo,
