...
const start = async (req, res, next) => {
  try {
    ...
    // Audit Log for Start Tryout
    if (req.user) {
      const logData = {
        id: req.user.id,
        name: req.user.name,
        email: req.user.email,
        url: `Mulai Mengerjakan: ${getPaketLatihan.nama}`,
        ip: req.ip,
...
const finish = async (req, res, next) => {
  try {
    ...
    // Audit Log for Finish Tryout
    if (req.user) {
      const logData = {
        id: req.user.id,
        name: req.user.name,
        email: req.user.email,
        url: `Selesai Mengerjakan: ${getTryout.paketLatihan?.nama || 'Tryout'}`,
        ip: req.ip,
        last_activity: Date.now(),
