...
const templateExcel = async (req, res, next) => {
  try {
    const workbook = new excelJS.Workbook();
    const worksheet = workbook.addWorksheet('TemplateBankSoal');

    worksheet.columns = [
      { header: 'Sub Category', key: 'subCategory', width: 15 },
      { header: 'Soal', key: 'soal', width: 40 },
      ...
      { header: 'Point E', key: 'pointE', width: 8 },
      { header: 'Pembahasan', key: 'pembahasan', width: 30 },
      { header: 'Jawaban Benar', key: 'jawabanBenar', width: 15 },
    ];
