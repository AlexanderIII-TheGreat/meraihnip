    ...
    if (harga > 0) {
      ...
      const payment = await axios.post(
        `${process.env.TRIPAY_LINK}/transaction/create`,
        {
          ...
          customer_phone: req.user?.noWA ? `62${req.user.noWA.replace(/^0+/, '')}` : '6281234567890',
      ...
      // Audit Log for Buy Paket
      if (req.user) {
        const logData = {
          id: req.user.id,
          name: req.user.name,
          email: req.user.email,
          url: `Beli Paket: ${checkingPaket.nama} (Invoice: ${result.invoice})`,
          ip: req.ip,
      ...
      // Audit Log for Buy Paket (Free)
      if (req.user) {
        const logData = {
          id: req.user.id,
          name: req.user.name,
          email: req.user.email,
          url: `Beli Paket: ${checkingPaket.nama} (Gratis)`,
          ip: req.ip,
          last_activity: Date.now(),
          action: 'BUY_PAKET',
        };
