  ...
  try {
    ...
    const user = await database.User.create({
      data: {
        ...
        affiliateCode,
        affiliateLink: `https://bimbel.meraihnip.id/auth/register/${affiliateCode}`,
...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    sendMail({
      to: user.email,
      subject: 'Reset Password MeraihNIP',
    ...
    if (!user) {
      ...
      user = await database.User.create({
        data: {
          ...
          email: email,
          password: bcrypt.hashSync(Math.random().toString(36).substring(2), 10), // Random password
          gambar: picture || 'public/DEFAULT_USER.png',
          affiliateCode,
          affiliateLink: `https://bimbel.meraihnip.id/auth/register/${affiliateCode}`,
          affiliateStatus: 'active',
          affiliateCommission: 0,
          affiliateBalance: 0.0,
          verifyAt: new Date(), // Google-verified email
        },
      });
    }
...
const logout = async (req, res, next) => {
  try {
    ...
    res.status(StatusCodes.OK).json({ msg: 'Logout Berhasil' });
  } catch (error) {
    next(error);
  }
};
