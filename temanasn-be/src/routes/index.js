...
const faqChatbotRouter = require('#api/faq-chatbot/route.js');
const faqChatbotAdminRouter = require('#api/faq-chatbot/route.admin.js');
const beritaRouter = require('#api/berita/route.js');
...
router.get('/api/health', async (req, res) => {
  try {
    ...
    res.status(500).json({ status: 'error', db: 'fail', ts: Date.now() });
  }
});
router.use('/api/auth', authRouter);
router.use('/api/dashboard', dashboardRouter);
router.use('/api/dashboard-notification', dashboardNotificationRouter);
router.use('/api/faq-chatbot', faqChatbotRouter);
...
router.use('/api/berita', beritaRouter);
...
router.use('/api/admin/faq-chatbot', faqChatbotAdminRouter);
